...
void radiodecay2(Double_t alpha1, Double_t alpha2, UInt_t pop, Double_t ttot, Double_t res){

	if(res<0||res>1)res=1;

	Double_t binbuffer,rndmbuffer;
	...
	for(Double_t i=0;i<=(ttot/res)&&i+2<=ttot;i+=res){
		...
		for(UInt_t j=0;j<=binbuffer;j++){
			...
			if(rndmbuffer<=(alpha1*res) || rndmbuffer<=(alpha2*res)){
				binbuffer-=2;
			}
		}
	...
	canv->SetTitle("decadimento radioattivo");
